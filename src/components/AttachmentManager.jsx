...
export const AttachmentManager = ({ attachments = [], links = [], onUpdate, t }) => {
    ...
    const handleFileUpload = (e) => {
        ...
        reader.onloadend = () => {
            ...
            onUpdate({ attachments: [...attachments, newAttachment], links });
            setError(null);
        };
                ...
                {activeTab === 'links' && (
                    <div className="space-y-4">
                        <div className="flex gap-2">
                            <input
                                placeholder="URL (Start with http://...)"
                                value={inputUrl}
                                onChange={e => setInputUrl(e.target.value)}
                                className="flex-1 p-2 bg-white border border-slate-200 rounded-lg text-sm"
                            />
                            <Button onClick={handleAddLink} disabled={!inputUrl} icon={Plus} className="px-3">Add</Button>
                        </div>
                        <input
